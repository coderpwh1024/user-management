# 用户管理服务 · 项目生成提示词（Project Prompt）

> 本文件是一份**可直接喂给 AI / 工程师**的项目脚手架提示词。它从现有 `user-management`
> 项目中提炼出技术栈、分层约定、编码规范，并融合**阿里巴巴《Java 开发手册》数据库规约**
> 与大厂（Google / 阿里 / 字节）的 API、命名、工程规范。
>
> 用法：把「角色设定 + 约束规范 + 交付清单」整段作为系统提示词（System Prompt），
> 再追加一句你的需求（如「新增一张 `t_role` 角色表并实现角色 CRUD」），即可生成
> 风格、规范完全一致的代码。
